...
### Level-up visuals
- **Level-up overlay** — if completing a quiz pushes to a new level, tapping "Back to World" triggers a full-screen overlay showing the new level icon and title. Auto-dismisses after 2.5s then navigates to the world screen.
- **Level pill pulse** — a gold ring pulse animation plays on the level pill every time she returns home after completing a unit.
- **Grand Quest bar animation** — the progress bar resets to 0 on each home render then animates to the real value, making progress visible as a sweep.
...
| 5 | XP + levelling visuals (level-up overlay, bar animation, pill pulse) | ✅ Done |
